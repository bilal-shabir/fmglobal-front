...
class O_M_error_dashboard extends React.Component {
  constructor(props) {
  ...
  returnButton = (row) => {
    ...
    return <button style={{ marginRight: "15px" }} onClick={this.ticketcloseModal}>Ticket</button>
  ...
  async getData() {
    ...
    let messages = [];
    let url = new URL(URL2 + `message/get`);
    await fetch(url, {
      headers: {
      ...
      const error_messages_ids = response.error_messages.map(message => message.id);

      messages = messages.filter(message => !error_messages_ids.includes(message.id));

      // // option 1
      messages.unshift(...response.error_messages);
        ...
        this.setState({
          ...
          columns: [
            ...
            },
            {
              ...
              // cell: (row) => this.returnOptions(row.id, row.message_status_name)
            },
            {
              button: true,
              cell: (row) => this.returnButton(row),
              grow: 0.5,
            }
          ],
          ...
          isLoaded: true,
          errors_inProgress: response.errors_inProgress,
          errors_new: response.errors_new,
          warnings_new: response.warnings_new,
          warnings_inProgress: response.warnings_inProgress,
          Info: response.Info,
          ok_checks: response.Normal,
        })
    })
  }
  ...
  processclickHandler = () => {
    ...
    //console.log("messages names", this.state.errormessage)
  }
  closeclickHandler = () => {
    this.setState({
      isOpenButton: true
  ...
  getStatus = async (event) => {
    ...
    const access_token = localStorage.getItem('access_token');
    await fetch(URL2 + 'message-status/get', {
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
        'access_token': access_token
  ...
  processAlert = async (event) => {
    ...
    var processId = 2 // This id is taken from the DB
    const processName = 'In-Progress' // This id is taken from the DB
      ...
      await fetch(URL2 + 'message/processAlert', {
        ...
        method: 'POST',
        mode: 'cors',
        body: JSON.stringify({ rows:this.state.selectedErrorMessages, statusId: processId, statusName: processName, comments: msg })
      })
        .then(response => response.json())
        .then((response) => {
          // console.log(response)
  ...
  handleRowSelected = event => {
    ...
    this.setState({
      selectedErrorMessages: event.selectedRows
    })

    console.log("Message State", this.state.selectedErrorMessages)
  ...
  render() {
    const { isLoaded , selectedErrorMessages} = this.state;
    return (
      <Container fluid className="main-content-container px-4">
        ...
        <span>
          {selectedErrorMessages? (
              ...
              <Modal.Footer>
                <Button onClick={this.TablecloseModal}>Close</Button>
              </Modal.Footer>
            </Modal>
          ) : (<L></L>)}
        ...
        <span>
          <Modal
            show={this.state.isOpenButton2}
            onHide={this.Tableclose}
            size="lg"
            aria-labelledby="contained-modal-title-vcenter"
            centered>
            <Modal.Header>
                  ...
                  <li class="list-group-item p-3">
                    <div class="row">
                      <div class="col-sm-12 col-md-12">
                        ...
                        <label class="form-label" for="msg">{this.state.msg_text}</label>

                      </div>

                      <div class="col-sm-12 col-md-12">

                        <label class="form-label" for="msg">Description</label>
                        <textarea class="form-control" id="msg" rows="8"></textarea>
            ...
            <Modal.Footer>
              <Button onClick={this.Tableclose}>Close</Button>
            </Modal.Footer>
          </Modal>
        </span>
            ...
            {isLoaded ? (
              <Card small>
                ...
                <Col lg="12" md="12" sm="12" className="mb-4">
                  ...
                  <button style={{ marginRight: "15px", float: "right" }} onClick={this.processclickHandler}>Process</button>
                  <button style={{ marginRight: "15px", float: "right" }} onClick={this.closeclickHandler}> Close</button>
                </Col>
...
export default O_M_error_dashboard;
