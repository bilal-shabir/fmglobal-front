...
class Pending_Approval_Invoices extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      companies: [],
      projects: [],
      operations_approval: false,
      finance_approval: false,
      cr_approval: false,
      invoice_id: null,
      approved_by_operation_single: null,
      customStyles: {
        headCells: {
          style: {
  ...
  async componentDidMount() {
    require("../../utils").checkpermision("Pending Approval");
    ...
    // if (
    //   permissions.find(
    //     ({ resource }) => resource.name === "operations_invoice_approval"
    //   )
    // ) {
    //   this.setState({
    ...
    // if (
    //   permissions.find(
    //     ({ resource }) => resource.name === "finance_invoice_approval"
    //   )
    // ) {
    //   this.setState({
    ...
    await fetch(url, {
      ...
      .then(response => {
        ...
        const invoices = response.filter(
          invoice => invoice.invoice_status.name == "Pending Approval"
        );
        var companies = [];
    ...
    fetch(URL2 + "invoice/getInvoice/" + id, {
      ...
      .then(response => {
        ...
        this.setState({
          ...
          approved_by_operation_single: response.invoice.approved_by_operation,
          show: true,
        });
        }
        //   console.log(response)
  ...
  operationapproval = async (id, action) => {
    document.getElementById("operationsapproval").disabled = true;
    document.getElementById("operationsrejection").disabled = true;

    if (this.state.approved_by_operation_single) {
      toast.info('Already Approved by Operations', {
        position: "top-center",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
      ...
      const access_token = localStorage.getItem("access_token");
      await fetch(
        URL2 + `invoice/operationApprovalForInvoice/${id}/${action}`,
        {
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
      ...
      .then(response => {
        if (response.statusCode == 404 || response.statusCode == 401 || response.statusCode == 500  || response.statusCode == 403 || response.statusCode == 400) { 
          ...
          if (action) {
            ...
            document.getElementById("operationsapproval").disabled = false;
            document.getElementById("operationsrejection").disabled = false;
      ...
      .catch((e) => {
        // console.log(e)  
        toast.error('Something went wrong, invoice not approved', {
            position: "top-center",
            autoClose: 3000,
            hideProgressBar: false,
            closeOnClick: true,
  ...
  financeapproval = async (id, action) => {
    document.getElementById("financeapproval").disabled = true;
    document.getElementById("financerejection").disabled = true;
    if (this.state.approved_by_operation_single) {
      const access_token = localStorage.getItem("access_token");
      await fetch(
        URL2 + `invoice/FinancialApprovalForInvoice/${id}/${action}`,
        {
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
      ...
      .then(response => {
        console.log(response)
        if (response.statusCode == 404 || response.statusCode == 401 || response.statusCode == 500 || Object.keys(response).length === 0 || response.statusCode == 403 || response.statusCode == 400) { 
          throw Error(response.statusText)        
        }
            else{
              if (action) {
                ...
                document.getElementById("financeapproval").disabled = false;
                document.getElementById("financerejection").disabled = false;
                this.handleClose();
              } else {
                toast.success('Invoice rejected', {
                  position: "top-center",
      ...
      toast.info('Pending Operations Approval', {
        position: "top-center",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
          ...
          <Can I="read" a="/invoice/getInvoices/" ability={this.state.ability}>
            ...
            <div class="card card-small mb-4" style={{ marginTop: "15px" }}>
              ...
              >
                ...
                {/* <button className="btn btn-primary" onClick={this.handleShowCreate} disabled={!this.state.inv_write} style={{backgroundColor:'#BDAC37', borderColor:'#BDAC37'}}>Create</button> */}
              </div>
            ...
            <>
              ...
              >
                ...
                <Modal.Footer>
                  ...
                  <Can I="write" a="/invoice/operationApprovalForInvoice/:id/:action/" ability={this.state.ability}>
                    <span>
                      <button
                        onClick={() =>
                          this.operationapproval(this.state.invoice_id, true)
                        }

                        id="operationsapproval"
                      ...
                      <button
                        onClick={() =>
                          this.operationapproval(this.state.invoice_id, false)
                        }

                        id="operationsrejection"
                        style={{
                          marginLeft: "5px",
                  ...
                  <Can I="write" a="/invoice/FinancialApprovalForInvoice/:id/:action/" ability={this.state.ability}>
                    <span>
                      <button
                        onClick={() =>
                          this.financeapproval(this.state.invoice_id, true)
                        }
                      ...
                      <button
                        onClick={() =>
                          this.financeapproval(this.state.invoice_id, false)
                        }
...
export default Pending_Approval_Invoices;
