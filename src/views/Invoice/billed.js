...
class Billed_Invoice extends React.Component {
            ...
            selector: row => (
              ...
              <span>
                ...
                <Link
                  ...
                  style={{ backgroundColor: "#BDAC37", borderColor: "#BDAC37" }}
                >
                  View PDF
                </Link>
              </span>
  ...
  resetfilters = () => {
    if (this.state.data.length != 0) {
      if (this.state.type == "employee") {
        ...
        document.getElementById("o_approval").selectedIndex = 0;
        document.getElementById("f_approval").selectedIndex = 0;
  ...
  reverse_invoice = async id => {
    const access_token = localStorage.getItem("access_token");
    await fetch(URL2 + `invoice/ReverseInvoice/${id}`, {
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
        access_token: access_token
    ...
    .then(response => response.json())
    .then(response => {
      if (response.statusCode == 404 || response.statusCode == 401 || response.statusCode == 500 || response.Status == 403 || response.statusCode == 400 ) { 
        throw Error(response.statusText)        
      }
      else{
    ...
    .catch((e) => {
      // console.log(e)  
      toast.error('Something went wrong, invoice not reversed', {
          position: "top-center",
          autoClose: 3000,
          hideProgressBar: false,
          closeOnClick: true,
              ...
              >
                ...
                <Modal.Footer>
                  ...
                  {this.state.type == "employee" ? (
                    <Can I="write" a="/invoice/ReverseInvoice/:id/" ability={this.state.ability}>
                    <button
                      // disabled={!this.state.reverse_access}
                      onClick={() =>
                        this.reverse_invoice(this.state.invoice[0].id)
                      }
                      style={{
                        marginLeft: "5px"
                      }}
